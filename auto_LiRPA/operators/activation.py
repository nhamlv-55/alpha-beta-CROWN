        ...
        if self.options.get('optimize_bound_args', {}).get('ob_beta', False):
            if self.options.get('optimize_bound_args', {}).get('ob_single_node_split', False):
                ...
                if type(A) is Patches:
                    ...
                    logger.debug("Somewhere deep inside bound_backward...")
                    logger.debug("A.shape{}".format(A.shape))
                    logger.debug("A.shape{}".format(A.shape))
                    logger.debug("sparse beta:\n{}".format(self.sparse_beta))
                    logger.debug("sparse beta sign\n{}".format (self.sparse_beta_sign))

                    #NHAM: same beta values are reused between inputs
